...
/**
 * Decode audio file to mono float32 PCM at 44100 Hz (matches worker expectations for OnsetRate).
 */
...
regenerate-default-features.cjs
  ...
  --merge <name>    Merge strategy (default: onsetRate)
`);
    process.exit(0);
  }

  if (args.merge !== 'onsetRate') {
    console.error(`Only --merge onsetRate is supported for now; got: ${args.merge}`);
    process.exit(1);
  }
  ...
  data.availableDataKeys = ensureAvailableKey(data.availableDataKeys || [], 'onsetRate');
