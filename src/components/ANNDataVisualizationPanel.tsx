...
// --- NEW: Add types for ANN-specific data ---
interface InferenceResult {
  predictedLabel: string;
  confidence?: number; // Optional confidence score
}

// --- Corrected Type for TrueLabelMap ---
interface TrueLabelMap {
  [songId: string]: string; // Map song ID to its true label string
}
// --- End Helper Types ---
...
interface VisualizationPanelProps {
  ...
  trueLabels: TrueLabelMap; // Use the corrected TrueLabelMap
  predictedLabels: Record<string, InferenceResult>; // Use InferenceResult
...
const ANNDataVisualizationPanel: React.FC<VisualizationPanelProps> = ({
  className,
  activeSongIds,
  songs,
  ...
  const [colorMode, setColorMode] = useState<'true' | 'predicted' | 'feature' | null>('true');
  // -------------------------------------------------
  ...
  const categoryValueMap = useMemo<CategoryValueMap>(() => {
    ...
    featureColumnsMap.categorical.forEach(category => {
      ...
      Object.values(songFeatures).forEach(features => {
        // Ensure features is not null and the category is a valid key
        if (features && Object.prototype.hasOwnProperty.call(features, category)) {
          const value = features[category as keyof Features]; // Cast is now safer after check
          if (typeof value === 'string') {
            uniqueValues.add(value);
          } else if (typeof value === 'number') {
            // Allow numbers if the feature can be numeric but used categorically (e.g., intensity)
  ...
  const getCategoricalValueForSong = (songId: string, category: string): string => {
    const features = songFeatures?.[songId]; // Use optional chaining

    if (!features) return 'N/A';

    // Check if category is a valid key before accessing
    if (!Object.prototype.hasOwnProperty.call(features, category)) {
      return 'N/A'; // Category doesn't exist for this feature set
    }

    const value = features[category as keyof Features]; // Cast is safer now
  ...
  // Helper to get true label for a song ID
  const getTrueLabelForSong = (songId: string): string | null => {
    return trueLabels?.[songId] ?? null;
  };

  // Helper to get predicted label and confidence for a song ID
  const getPredictionForSong = (songId: string): InferenceResult | null => {
    return predictedLabels?.[songId] ?? null;
  };
  ...
  const createDetailedHoverText = (songId: string, songName: string): string => {
    let hoverText = `<b>${songName}</b>`;
    const trueLabel = getTrueLabelForSong(songId);
    const prediction = getPredictionForSong(songId);
    const clusterAssignment = kmeansAssignments?.[songId];

    if (clusterAssignment !== undefined) {
      hoverText += `<br><b>Cluster:</b> ${clusterAssignment}`;
    }

    if (trueLabel) hoverText += `<br><b>True Label:</b> ${trueLabel}`;

    if (showPredictions && prediction) {
      hoverText += `<br><b>Predicted:</b> ${prediction.predictedLabel}`;
      if (prediction.confidence !== undefined) {
        hoverText += ` (Conf: ${(prediction.confidence * 100).toFixed(1)}%)`;
      }
    }
  ...
  const plotDataAndLayout = useMemo(() => {
    try {
      ...
      const getAxisTitle = (sel: string | null, defaultPrefix: string): string => {
        ...
        if (selectedDataStage === 'reduction') {
      ...
      // Prepare intermediate point representation with label info
      ...
      filteredSongIds.forEach(id => {
        const pointVector = dataPoints[id];
        const song = songMap.get(id);

        if (pointVector && pointVector.length > maxRequiredDimIndex && song) {
          const trueLabel = getTrueLabelForSong(id);
          const prediction = getPredictionForSong(id);
          const colorFeatureKey = selectedColorBy?.startsWith('feature:') ? selectedColorBy.substring(8) : null;
          const colorFeatureValue = colorFeatureKey ? getCategoricalValueForSong(id, colorFeatureKey) : null;
          const clusterAssignment = kmeansAssignments?.[id];

          intermediatePoints.push({
            x: pointVector[xAxisIndex],
            y: pointVector[yAxisIndex],
            z: selectedDimensions === 3 ? pointVector[zAxisIndex] : undefined,
            id: id,
            name: song.name,
            cluster: clusterAssignment,
            trueLabel: trueLabel,
            predictedLabel: prediction?.predictedLabel ?? null,
            colorFeatureValue: (colorFeatureValue && colorFeatureValue !== 'N/A') ? colorFeatureValue : null,
          });
        } else {
          // console.warn(`[Plot Memo] Skipping song ${id}: Point data missing or insufficient dimensions`);
        }
      });
      ...
      const colorByTrueLabel = colorMode === 'true';
      const colorByPredictedLabel = colorMode === 'predicted';
      ...
      intermediatePoints.forEach(point => {
        ...
        if (colorByTrueLabel && point.trueLabel) {
          groupName = point.trueLabel;
          pointColorNeedsAssign = !categoryToColor[groupName];
        } else if (colorByPredictedLabel && point.predictedLabel) {
          groupName = point.predictedLabel;
          pointColorNeedsAssign = !categoryToColor[groupName];
        } else if (colorByCategorical && categoryKey && point.colorFeatureValue && point.colorFeatureValue !== 'N/A') {
          groupName = point.colorFeatureValue;
          pointColorNeedsAssign = !categoryToColor[groupName];
        }
      ...
      const sortedGroupNames = Object.keys(groupedPoints).sort((a, b) => {
      ...
      sortedGroupNames.forEach(groupName => {
        const pointsInGroup = groupedPoints[groupName];
        if (pointsInGroup.length === 0) return;
        const traceColor = categoryToColor[groupName];

        const trace: Partial<Plotly.PlotData> = {
          // --- Filter out potential undefined coordinates --- 
          x: pointsInGroup.map(p => p.x).filter((v): v is number => v !== undefined),
          y: pointsInGroup.map(p => p.y).filter((v): v is number => v !== undefined),
          type: traceType,
          mode: 'markers',
          marker: { color: traceColor, size: 8, opacity: 0.8 },
          text: pointsInGroup.map(p => createDetailedHoverText(p.id, p.name)),
          hoverinfo: 'text',
          name: groupName,
          showlegend: true
        };
    ...
    selectedDataStage, unprocessedData, processedData, reducedDataPoints, reductionDimensions, activeSongIds, songMap,
    ...
    isReducedDataAvailable, isProcessedDataAvailable, isUnprocessedDataAvailable, trueLabels, predictedLabels, showPredictions, colorMode
  ]);
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs mb-2">
          ...
          <div className="flex items-center gap-1">
            ...
            >
              <option value="raw" disabled={!isUnprocessedDataAvailable}>Raw Features</option>
              <option value="processed" disabled={!isProcessedDataAvailable}>Processed Data</option>
              <option value="reduction" disabled={!isReducedDataAvailable}>Reduced Data</option>
            </select>
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            <select
              ...
              disabled={!isUnprocessedDataAvailable && !isProcessedDataAvailable}
            >
...
export default ANNDataVisualizationPanel;
